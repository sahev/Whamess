...
namespace whamess
{
    ...
    class SendMessage
    {
        ...
        public void AttachNavRequest()
        {
            ...
            string jstext = " var eventFire = (MyElement, ElementType) => { var MyEvent = document.createEvent(\"MouseEvents\"); MyEvent.initMouseEvent (ElementType, true, true, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null); MyElement.dispatchEvent(MyEvent); }; function myFunc() { messageBox = document.querySelectorAll(\"[contenteditable='true']\")[1]; message = \"" + text + "\"; event = document.createEvent(\"UIEvents\"); messageBox.innerHTML = message.replace(/ /gm, ' '); event.initUIEvent(\"input\", true, true, window, 1); messageBox.dispatchEvent(event); eventFire(document.querySelector('span[data-icon=\"send\"]'), 'click'); }; myFunc(); ";
